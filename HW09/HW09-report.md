...
--Смущает наличие в теоретическом плане строки Disabled: true. Говорит о том, что планировщик вынужден использовать запрещенную Node в плане - SeqScan, т.к. другие использовать не может (индекс). Может говорить о преобразовании типов данных..постараюсь найти время для исследования, но пока не получилось продемонстирировать (если сможете указать причину проблемы - буду признателен). По теории, в первой сесии обновление будет построчно раз в 3 секунды с сортировкой ASC (по SeqSCan), во второй сесии будет обновление по IndexScan с сортировкой DESC. Взаимоблокировка предполагается будет на второй строке.
```
