...
--Не сторонник добавлять DEFAULT партицию (потребляет лишние ресурсы при выполнении запросов и усложняет план выполнения оптимизатором).
--При этом необходимо обеспечивать наличие нужной партиции (например, через pg_cron).
```
