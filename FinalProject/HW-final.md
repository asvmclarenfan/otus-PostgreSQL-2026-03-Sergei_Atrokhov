...
$(lsb_release -cs) — подстановка короткого названия ОС (например, ubuntu24.04 итд). Скрипт динамически определяет нужную папку с пакетами на сервере разработчиков.
tee записывает вывод echo в файл от имени суперпользователя (sudo), так как обычные пользователи не имеют прав изменять системные конфиги.
